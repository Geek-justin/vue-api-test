<template>
    <el-row class="title-section">
        <el-col :span="20">
            <el-breadcrumb separator="/" v-show="!state.nameEditMode">
                <el-breadcrumb-item v-for="item in state.parents" :key="item">{{ item.name }}</el-breadcrumb-item>
                <el-breadcrumb-item>
                    {{ state.name }}
        ...
        <el-col :span="4">
            ...
            <el-select v-model="vars.env" style="margin-left:10px; width: 120px">
                <el-option v-for="(v, e) in vars.map" :label="e" :value="e" :key="e"></el-option>
            </el-select>
    ...
    <el-row class="func-section">
        ...
        <el-col :span="12">
            <response-content :content="state.responseContent"></response-content>
        </el-col>
    </el-row>
...
<script>
import { defineComponent, ref, reactive, watchEffect, toRaw } from 'vue'
import { ElMessage } from 'element-plus'
import axios from 'axios'
import ApiDB from '../lib/db.js';
    ...
    setup(props, context) {
        ...
        function updateUrlRaw(payload) {
            ...
            if (state.url.host.length > 0) {
                _url = toRaw(state.url.host).join('.')
                _url = decodeURI(_url)
            }
        ...
        function saveConfig() {
            let config = {
                url: state.rawUrl,
                method: state.method,
                headers: toRaw(state.headers),
                body: toRaw(state.body),
            }
            console.dir(config)
        ...
        function sendApiRequest() {
            let _url = '',
                _protocol = toRaw(state.url.protocol),
                _domain = '',
                _path = '/',
                _method = toRaw(state.method),
                _headers = {
                    Accept: '*/*',
                    'Content-Type': '',
                },
            ...
            if (state.url.host.length > 0) {
                _domain = toRaw(state.url.host).join('.')
                _domain = replaceVarByEnv(_domain)
            }
            if (state.url.path.length > 0) {
                _path += toRaw(state.url.path).join('/')
