<template>
    <el-row class="height-full">
        <el-col :span="4"  class="height-full">
                    ...
                    <template #default="{ node, data }">
                        <span class="custom-tree-node">
                            ...
                            <span class="context-menu" v-if="data.config">
                                <a @click="removeCommon(node, data)" style="color:red">
                                ...
                                <el-dropdown trigger="click" size="small">
                                    ...
                                    <template #dropdown>
                                        <el-dropdown-menu>
                                            ...
                                            <el-dropdown-item
                                                @click="addRequest(node, data)"
                                            >Add request</el-dropdown-item>
                                            <el-dropdown-item
                                                v-if="data.id != 1"
                                                @click="removeCommon(node, data)"
                                                style="color:red"
        ...
        <el-col :span="20" class="height-full">
            <api-group
                :apis="state.apis"
                :active="state.active"
...
export default defineComponent({
    ...
    setup() {
        ...
        function syncUpdateDatabase() {
            let api = toRaw(state.data)
            ApiDB.setStorage('api', api)
        }
        function editFolder(_node, _object) {
            ElMessageBox.prompt('Edit folder name', '', {
        ...
        function addFolder(_node, _object) {
            if (typeof _object['config'] == 'undefined') {
                ...
                ElMessageBox.prompt('Input folder name', '', {})
                    .then(({ value }) => {
                        if (value) {
        ...
        function handleSaveConfig(payload) {
            let index = payload['index']
            let config = toRaw(payload['config'])
            let target = state.apis[index]
        ...
        function closeAll() {
            ElMessageBox.confirm('Close all ?', '', {}).then(() => {
                state.apis = []
                state.openedList = []
            })
        }
